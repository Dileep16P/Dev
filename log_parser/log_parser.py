...
#Image List (get_image_list):

def get_image_list(log_file):
    words_list = log_file.split()
    image_list = [word for word in words_list if word.lower().endswith(('.gif', '.jpeg', '.png', '.jpg'))]
    return image_list

#No of Images Count (count_image):

def count_image(image_list):
    return len(image_list)

#Images Counter (counter_image):

def counter_image(image_list):
    from collections import Counter
    return Counter(image_list)

#Email Counter (counter_email):

def counter_email(email_list):
    from collections import Counter
    return Counter(email_list)

#IP Address JSON (create_json_file_ip):

def create_json_file_ip(filename, ip_list):
    data = {"IP Addresses": ip_list}
    with open(filename, 'w') as json_file:
        json.dump(data, json_file, indent=2)

#Email JSON (create_json_file_email):

def create_json_file_email(filename, email_list):
    data = {"Emails": email_list}
    with open(filename, 'w') as json_file:
        json.dump(data, json_file, indent=2)

#Date List (get_date_list):

def get_date_list(log_file):

    return date_list

#URL List (get_url_list):

def get_url_list(log_file):

    url_list = []
    return url_list

#Date Counter (counter_date):

def counter_date(date_list):
    from collections import Counter
    return Counter(date_list)

#URL Counter (counter_url):

def counter_url(url_list):
    from collections import Counter
    return Counter(url_list)

#Date JSON (create_json_file_date):

def create_json_file_date(filename, date_list):
    data = {"Dates": date_list}
    with open(filename, 'w') as json_file:
        json.dump(data, json_file, indent=2)


#URL JSON (create_json_file_url):

def create_json_file_url(filename, url_list):
    data = {"URLs": url_list}
    with open(filename, 'w') as json_file:
        json.dump(data, json_file, indent=2)
